...
The project is designed to run on [Xilinx ZC706 evaluation kit](https://www.xilinx.com/products/boards-and-kits/ek-z7-zc706-g.html) but with minor changes it can be ported to [PicoZed](https://www.avnet.com/wps/portal/us/products/avnet-boards/avnet-board-families/picozed/) with [FMC carrier card](https://www.avnet.com/shop/us/products/avnet-engineering-services/aes-pzcc-fmc-v2-g-3074457345635014223), cheap development boards [from AliExpress](https://www.aliexpress.com/item/1005001501660085.html) and others platforms based on Zynq-7000 series SoC.
 ...
 * Python tools that are using [Linux Userspace I/O framework](https://www.kernel.org/doc/html/v4.14/driver-api/uio-howto.html), for more information see `python/linux_uio.py` and `python/linux_axi_dma.py` files.

 * Loadable kernel module `zc_dma_mem.ko` that works with transaction layler of PCI Express and provides an access to the target system physical memory using character device based API, for more information see `kernel/zc_dma_mem/zc_dma_mem.c` file.
...
The project is consists from the following files:
 ...
 * `devicetree` &minus; Files required to generate device tree for Linux kernel running on Zynq-7000 processing system.
...
Compile kernel, bootloder and `zc_dma_mem.ko` kernel extension:
...
To prepare root file system with Ubuntu 20.04 we will use debootstrap and QEMU userspace emulation:
...
Fetch Ubuntu 20.04 root filesystem with debootstrap:
...
Now your Ubuntu system for Xilinx Zynq is almost ready, let's do some basic configuration.
Create `/etc/fstab`:
...
Run fdisk and create two partitios, 256 Megabytes will be used for boot partition and remaining SD card space for root filesystem:
...
Create FAT32 filesystem for boot partition:
...
Mount root partition and copy needed files:
...
$ sudo mkdir -p /mnt/tmp/home/user/zc_pcie_dma
$ sudo cp -r ~/zc_pcie_dma/python /mnt/tmp/home/user/zc_pcie_dma
$ sudo chown -R 1000:1000 /mnt/tmp/home/user/zc_pcie_dma
$ sudo umount /mnt/tmp
```

Eject SD card, insert it into the board:

```
$ sudo eject /dev/sdc
```

Now you can connect USB cable to the serial console of ZC706 and turn the board on:
...
After the Linux is successfully booted you can log into the system with previously created user and try to load `zc_dma_mem.ko` kernel extension:
...
Now you have everything what you need to perform some DMA attacks.


## DMA attacks usage

ZC706 board must be connected to PCIe (or mPCIe, or M.2) slot of the target system before power on. After the target system was booted you can see the following PCI Express device connected:
...
As it was said previously, Python tools can interact with programmable logic using Linux UIO framework or, more preferable, loadable kernel module. To use `zc_dma_mem.ko` kernel module you need to edit `python/pcie_lib_config.py` and set `device_type` variable to `DEVICE_TYPE_DRIVER`. To use UIO you need to set `device_type` variable to `DEVICE_TYPE_UIO`.
